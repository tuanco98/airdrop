...
const twit = new Twit({
    ...
    consumer_secret: TWITTER_APPLICATION_CONSUMER_SECRET,
...
export const checkRetweet = async () => {
    try {
        const data = (await new Promise((res, rej) => {
            ...
            created_at: string
            text: string
            entities: { hashtags: any[]; user_mentions: any[] }
...
class TwitterDev2 {
    ...
    public getTweets = async (ids: string[]) => {
        try {
            const tweetResponse = await this._twit.readOnly.v2.tweets(ids, {
...
class TwitterDev {
    ...
    public getTweetsHasHashtag = async (hashtag: string) => {
        try {
            const tweetResponse = await this._twit.get(
                `search/tweets.json?q=${hashtag}&include_entities=true&count=100`
            )
...
export const twitter_dev_2 = new TwitterDev2(TWITTER_BEARER_TOKEN)
